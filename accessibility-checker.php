...
// SVG Icons.
// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- This is a local file, no need to use wp_remote_get.
...
function edac_register_rules() {
	$rules = include __DIR__ . '/includes/default-rules.php';

	// filter rules.
	return apply_filters( 'edac_filter_register_rules', $rules );
}
...
function edac_summary_ajax() {
	...
	if ( ! isset( $_REQUEST['nonce'] ) || ! wp_verify_nonce( sanitize_key( $_REQUEST['nonce'] ), 'ajax-nonce' ) ) {
		wp_send_json_error(
			new WP_Error( '-1', esc_html__( 'Permission Denied', 'accessibility-checker' ) )
		);
	}
	...
	if ( 'none' !== $simplified_summary_prompt ) {
		...
		if ( $summary['content_grade'] <= 9 ) {
			...
			$simplified_summary_text = $summary['simplified_summary']
				? esc_htm__( 'A Simplified summary has been included for this content.', 'accessibility-checker' )
				: esc_html__( 'A Simplified summary has not been included for this content.', 'accessibility-checker' );
		}
	} else {
	...
	$html['content'] .= '<div class="edac-summary-total-progress-circle ' . ( ( $summary['passed_tests'] > 50 ) ? ' over50' : '' ) . '">
		<div class="edac-summary-total-progress-circle-label">
			...
			<div class="edac-panel-number-label">' . sprintf(
				/* translators: %s: <sup>*</sup> */
				esc_html__( 'Passed Tests%s', 'accessibility-checker' ),
				'<sup>*</sup>'
			) . '</div>
		</div>
	...
	<div class="edac-summary-stats">
		...
		<div class="edac-summary-stat edac-summary-contrast' . ( ( $summary['contrast_errors'] > 0 ) ? ' has-errors' : '' ) . '">
			...
			<div class="edac-panel-number-label">' . _n( 'Contrast Error', 'Contrast Errors', $summary['contrast_errors'], 'accessibility-checker' ) . '</div>
		</div>
		<div class="edac-summary-stat edac-summary-warnings' . ( ( $summary['warnings'] > 0 ) ? ' has-warning' : '' ) . '">
			<div class="edac-panel-number">
				' . $summary['warnings'] . '
			</div>
			<div class="edac-panel-number-label">' . _n( 'Warning', 'Warnings', $summary['warnings'], 'accessibility-checker' ) . '</div>
		</div>
		<div class="edac-summary-stat edac-summary-ignored">
			<div class="edac-panel-number">
				' . $summary['ignored'] . '
			</div>
			<div class="edac-panel-number-label">' . _n( 'Ignored Item', 'Ignored Items', $summary['ignored'], 'accessibility-checker' ) . '</div>
		</div>
	</div>
	<div class="edac-summary-readability">
		<div class="edac-summary-readability-level">
			...
			<div class="edac-panel-number-label' . ( ( (int) $summary['readability'] <= 9 || 'none' === $simplified_summary_prompt ) ? ' passed-text-color' : ' failed-text-color' ) . '">' . sprintf(
					/* translators: %s: <br> */
					esc_html__( 'Reading %sLevel', 'accessibility-checker' ),
					'<br>'
				) . '</div>
		</div>
	...
	<div class="edac-summary-disclaimer"><small>' . sprintf(
				/* translators: %s: "Learn how to manually test for accessibility" link. */
				esc_html__( '* True accessibility requires manual testing in addition to automated scans. %s.', 'accessibility-checker' ),
				'<a href="https://a11ychecker.com/help4280">' . esc_html__( 'Learn how to manually test for accessibility', 'accessibility-checker' ) . '</a>'
			) . '</small></div>';

	if ( ! $html ) {
		wp_send_json_error( new WP_Error( '-3', esc_html__( 'No summary to return', 'accessibility-checker' ) ) );
	}
