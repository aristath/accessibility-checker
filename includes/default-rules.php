...
return array(
	array(
		...
		'rule_type' => 'error',
		'summary'   => sprintf(
			// translators: %s is the code for an image tag.
			esc_html__( 'An Image Missing Alternative Text error means that your image does not have an alt attribute (alt="") contained in the image tag (%s) at all. To fix an Image Missing Alternative Text error, you will need to add an alt tag to the image with appropriate text describing the purpose of the image in the page. If the image is decorative, the alt attribute can be empty, but the HTML alt="" tag still needs to be present.', 'accessibility-checker' ),
			'<code><img/></code>'
		),
	),
	array(
	...
	array(
		...
		'rule_type' => 'error',
		'summary'   => sprintf(
			// translators: %1$s is the code for an h3 tag, %2$s is the code for an h1 tag, %3$s is the code for an h2 tag.
	...
	array(
		...
		'rule_type' => 'warning',
		'summary'   => sprintf(
			// translators: %1$s is the code for an h1 tag, %2$s is the code for an h6 tag.
	...
	array(
		...
		'rule_type' => 'warning',
		'summary'   => sprintf(
			// translators: %s is a link to the Accessibility New Window Warnings plugin.
			...
			'<a href="https://wordpress.org/plugins/accessibility-new-window-warnings/" target="_blank">' . esc_html__( 'Accessibility New Window Warnings plugin.', 'accessibility-checker' ) . '</a>'
		),
	),
	array(
		...
		'rule_type' => 'error',
		'summary'   => esc_html__( 'The Image Map Missing Alternative Text error means that one of the <area> elements within your image map does not have alternative text added in an alt="" attribute. To fix this error, you will need to add missing alt text to any area tags that do not have alt text. The alt text needs to describe the function of the link contained in the area, not necessarily describe what the area looks like.', 'accessibility-checker' ),
	),
	array(
		...
		'rule_type' => 'warning',
		'summary'   => esc_html__( 'A Tab Order Modified Warning appears when the logical tab order on the page has been changed by adding an attribute for tabindex that is greater than 0 to an HTML element (for example, tabindex="1"). This can cause navigation issues for keyboard-only users. To resolve a Tab Order Modified warning you need to view the front end of your website on the page or post where the tab order has been modified and test to see if the modification is correct or not. If the tab order modification does not cause problems, then you can "Ignore" the warning. If the modified tab order causes information to be presented out of order, then you need to remove the tabindex attribute from the flagged element.', 'accessibility-checker' ),
	),
	array(
		...
		'rule_type' => 'error',
		'summary'   => sprintf(
			// translators: %s is the code for an h1 tag.
	...
	array(
		...
		'rule_type' => 'error',
		'summary'   => sprintf(
			// translators: %1$s is the code for a button tag, %2$s is the code for an input tag.
	...
	array(
		...
		'rule_type' => 'error',
		'summary'   => sprintf(
			// translators: %1$s is the code for a th tag, %2$s is the code for a th tag along with its closing tag.
	...
	array(
		...
		'rule_type' => 'error',
		'summary'   => esc_html__( 'The Long Description Invalid error means that a long description attribute (longdesc="") on an image does not have an appropriate URL, filename, or file extension. It may also mean that the long description is not a URL, or it has been left blank. The longdesc attribute is not fully supported opens a new window by HTML5, browsers, and all screen readers. Due to this lack of support, the best fix for this error is to remove longdesc from your image tag completely.', 'accessibility-checker' ),
	),
	array(
		...
		'rule_type' => 'error',
		'summary'   => sprintf(
			// translators: %s is the code for a label tag.
			esc_html__( 'An Empty Form Label error is triggered when a %s tag is present in your form and associated with an input (form field), but does not contain any text. To fix empty form label errors, you\'ll need to determine how the field and form were created and then add text to the label for the field that is currently blank.', 'accessibility-checker' ),
			'<code><label></code>'
		),
	),
	array(
		...
		'rule_type' => 'error',
		'summary'   => sprintf(
			// translators: %1$s is the code for an input tag, %2$s is the code for a label tag.
			esc_html__( 'A Missing Form Label error is triggered when an %1$s (form field) is present in your form and but is not associated with a %2$ element. This could mean the label is present but is missing a for="" attribute to connect it to the applicable field or there could be no label present at all and only an %1$s tag. To fix missing form label errors, you\'ll need to determine how the field and form were created and then add field labels or a correct for="" attribute to exisiting labels that are not connected to a field.', 'accessibility-checker' ),
			'<code><input></code>',
			'<code><label></code>'
		),
	),
	...
	array(
		...
		'rule_type' => 'warning',
		'summary'   => sprintf(
			// translators: %s is the code for a "u" tag.
	...
	array(
		...
		'rule_type' => 'error',
		'summary'   => sprintf(
			// translators: %1$s is the code for a blink tag, %2$s is the code for a marquee tag, %3$s is the code for a CSS text-decoration: blink.
	...
	array(
		...
		'rule_type' => 'error',
		'summary'   => sprintf(
			// translators: %1$s is the code for a # tag, %2$s is the code for role="button", %3$s is the code for a button tag, %4$s is the code for role="button", %5$s is the code for an a tag.
			esc_html__( 'An Improper Use of Link error appears if you have links that are missing an href attribute or are only linked to a %1$s, and do not have %2$s on them. Links should be used to direct people to other parts of your site. Any other functionality that triggers an action should be a button. To resolve this error you need to recode the link to use a %3$s tag (preferable) or add %4$s to the existing %5$s tag. If the element is a toggle button (such as an accordion), additional ARIA attributes are required.', 'accessibility-checker' ),
			'<code>#</code>',
